...
const git = src_default()({ baseDir });
console.log(`Running in ${baseDir}`);
(() => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    yield checkInputs().catch(core.setFailed);
    (0,core.startGroup)('Internal logs');
    (0,core.info)('> Staging files...');
    if (getInput('add')) {
        (0,core.info)('> Adding files...');
        yield add();
    }
    else
        (0,core.info)('> No files to add.');
    if (getInput('remove')) {
        (0,core.info)('> Removing files...');
        yield remove();
    }
    else
        (0,core.info)('> No files to remove.');
    (0,core.info)('> Checking for uncommitted changes in the git working tree...');
    const changedFiles = (yield git.diffSummary(['--cached'])).files.length;
    if (changedFiles > 0) {
        (0,core.info)(`> Found ${changedFiles} changed files.`);
        yield git
            .addConfig('user.email', getInput('author_email'), undefined, log)
            .addConfig('user.name', getInput('author_name'), undefined, log);
        (0,core.debug)('> Current git config\n' +
            JSON.stringify((yield git.listConfig()).all, null, 2));
        yield git.fetch(['--tags', '--force'], log);
        (0,core.info)('> Switching/creating branch...');
        yield git
            .checkout(getInput('branch'), undefined, log)
            .catch(() => git.checkoutLocalBranch(getInput('branch'), log));
        if (getInput('pull_strategy') == 'NO-PULL')
            (0,core.info)('> Not pulling from repo.');
        else {
            (0,core.info)('> Pulling from remote...');
            yield git.fetch(undefined, log).pull(undefined, undefined, {
                [getInput('pull_strategy')]: null
            }, log);
        }
        (0,core.info)('> Re-staging files...');
        if (getInput('add'))
            yield add({ ignoreErrors: true });
        if (getInput('remove'))
            yield remove({ ignoreErrors: true });
        (0,core.info)('> Creating commit...');
        yield git.commit(getInput('message'), undefined, Object.assign({ '--author': `"${getInput('author_name')} <${getInput('author_email')}>"` }, (getInput('signoff')
            ? {
                '--signoff': null
            }
            : {})), (err, data) => {
        ...
        if (getInput('tag')) {
            ...
            yield git
                ...
                .catch((err) => (0,core.setFailed)(err));
        }
        else
            (0,core.info)('> No tag info provided.');
        const pushOption = (_a = parseBool(getInput('push'))) !== null && _a !== void 0 ? _a : getInput('push');
        if (pushOption) {
            // If the options is `true | string`...
            (0,core.info)('> Pushing commit to repo...');
            if (pushOption === true) {
                (0,core.debug)(`Running: git push origin ${getInput('branch')} --set-upstream`);
                yield git.push('origin', getInput('branch'), { '--set-upstream': null }, (err, data) => {
                    if (data)
                ...
                (0,core.debug)(`Running: git push ${pushOption}`);
                yield git.push(undefined, undefined, matchGitArgs(pushOption), (err, data) => {
                    if (data)
                        setOutput('pushed', 'true');
                    return log(err, data);
                });
            ...
            if (getInput('tag')) {
                (0,core.info)('> Pushing tags to repo...');
                yield git
                    .pushTags('origin', undefined, (e, d) => log(undefined, e || d))
                    .catch(() => {
                    (0,core.info)('> Tag push failed: deleting remote tag and re-pushing...');
                    return git
                        .push(undefined, undefined, {
                        '--delete': null,
                        origin: null,
                        [matchGitArgs(getInput('tag')).filter((w) => !w.startsWith('-'))[0]]: null
                    }, log)
                        .pushTags('origin', undefined, log);
                });
            }
            else
                (0,core.info)('> No tags to push.');
        }
        else
            (0,core.info)('> Not pushing anything.');
        (0,core.endGroup)();
        (0,core.info)('> Task completed.');
    }
    else {
        (0,core.endGroup)();
        (0,core.info)('> Working tree clean. Nothing to commit.');
    }
}))()
    .then(logOutputs)
    .catch((e) => {
...
function checkInputs() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (getInput('add')) {
            ...
            if (parsed.length == 1)
                (0,core.info)('Add input parsed as single string, running 1 git add command.');
            else if (parsed.length > 1)
                (0,core.info)(`Add input parsed as string array, running ${parsed.length} git add commands.`);
            else
                (0,core.setFailed)('Add input: array length < 1');
        }
        if (getInput('remove')) {
            const parsed = parseInputArray(getInput('remove'));
            if (parsed.length == 1)
                (0,core.info)('Remove input parsed as single string, running 1 git rm command.');
            else if (parsed.length > 1)
                (0,core.info)(`Remove input parsed as string array, running ${parsed.length} git rm commands.`);
            else
                (0,core.setFailed)('Remove input: array length < 1');
        }
        // #endregion
        ...
        (0,core.info)(`> Using '${getInput('author_name')} <${getInput('author_email')}>' as author.`);
        // #endregion
        // #region message
        setDefault('message', `Commit from GitHub Actions (${process.env.GITHUB_WORKFLOW})`);
        (0,core.info)(`> Using "${getInput('message')}" as commit message.`);
        // #endregion
        // #region branch
        const branch = setDefault('branch', defaultBranch || '');
        if (isPR)
            (0,core.info)(`> Running for a PR, the action will use '${branch}' as ref.`);
        // #endregion
        // #region signoff
        if (getInput('signoff')) {
            ...
            (0,core.debug)(`Current signoff option: ${getInput('signoff')} (${typeof getInput('signoff')})`);
        }
        // #endregion
        // #region pull_strategy
        if (getInput('pull_strategy') == 'NO-PULL')
            (0,core.debug)("NO-PULL found: won't pull from remote.");
        // #endregion
        // #region push
        if (getInput('push')) {
            // It has to be either 'true', 'false', or any other string (use as arguments)
            const parsed = parseBool(getInput('push'));
            (0,core.debug)(`Current push option: '${getInput('push')}' (parsed as ${typeof parsed})`);
        }
    });
}
function add({ logWarning = true, ignoreErrors = false } = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const args of parsed) {
            ...
            yield git
                .add(matchGitArgs(args), (err, data) => log(ignoreErrors ? null : err, data))
                .catch((e) => {
                if (ignoreErrors)
                    return;
                if (e.message.includes('fatal: pathspec') &&
                    ...
                    logWarning)
                    (0,core.warning)(`Add command did not match any file:\n  git add ${args}`);
                else
                    throw e;
            }));
        }
...
function remove({ logWarning = true, ignoreErrors = false } = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const args of parsed) {
            ...
            // If any of them fails, the whole function will return a Promise rejection
            yield git
                .rm(matchGitArgs(args), (e, d) => log(ignoreErrors ? null : e, d))
                .catch((e) => {
                if (ignoreErrors)
                    return;
                if (e.message.includes('fatal: pathspec') &&
                    e.message.includes('did not match any files'))
                    logWarning &&
                        (0,core.warning)(`Remove command did not match any file:\n  git rm ${args}`);
                else
                    throw e;
            }));
        }
...
function parseInputArray(input) {
    try {
        ...
        if (json &&
            ...
            (0,core.debug)(`Input parsed as JSON array of length ${json.length}`);
            return json;
        }
    }
    catch (_a) { }
    try {
        const yaml = js_yaml.load(input);
        if (yaml &&
            Array.isArray(yaml) &&
            yaml.every((e) => typeof e == 'string')) {
            (0,core.debug)(`Input parsed as YAML array of length ${yaml.length}`);
            return yaml;
        }
    }
    catch (_b) { }
    (0,core.debug)('Input parsed as single string');
    return [input];
}
function logOutputs() {
    (0,core.startGroup)('Outputs');
    for (const key in outputs) {
        (0,core.info)(`${key}: ${outputs[key]}`);
    }
    (0,core.endGroup)();
}
...
/******/ 	return __nccwpck_require__(5142);
/******/ })()
;